...
impl OpenAIPreprocessor {
        ...
        Resp: Send + Sync + Clone + 'static + std::fmt::Debug,
    {
            ...
            Resp: Send + Sync + Clone + 'static + std::fmt::Debug,
        {
            ...
            trace_finish_reason_metadata: Option<crate::request_trace::SharedFinishReasonMetadata>,
            mm_counts: MultimodalCounts,
            // Counts are request-constant; emit them on the first metrics frame only.
            mm_counts_emitted: bool,
        }
            ...
            async move {
                ...
                if let Some(mut response) = inner.response_stream.next().await {
                    ...
                    let decode_worker_type = tracker
                        ...
                        .map(String::from);
                    // Emit request-constant counts on the first metrics frame only.
                    let mm_counts = if inner.mm_counts_emitted {
                        MultimodalCounts::default()
                    } else {
                        inner.mm_counts_emitted = true;
                        inner.mm_counts
                    };
