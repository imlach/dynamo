...
If the transfer is absent or fails, the worker falls back to loading the
original media URL. P/D prefill deliberately uses the original media because
it still needs raw-media-derived metadata for the decode handoff.
