...
impl TopologyAdapter {
    ...
    pub fn spawn(
        ...
        let task = tokio::spawn(async move {
            ...
            // Once the backend change sender drops, `changed()` returns `Err`
            // instantly; the guard disables that arm so we don't busy-loop and
            // keep reacting to pod changes.
            let mut backend_alive = true;
            loop {
                reconcile_once(&reflector, backend.as_ref(), &defaults).await;
                // Re-reconcile on a pod change or a selector-fleet change. Exit
                // if the pod-change sender drops (reflector gone).
                tokio::select! {
                    ...
                    changed = backend_changes.changed(), if backend_alive => {
                        if changed.is_err() {
                            backend_alive = false;
                            tracing::debug!("Backend change channel closed; reacting to pod changes only");
                        }
                    }
                }
            }
