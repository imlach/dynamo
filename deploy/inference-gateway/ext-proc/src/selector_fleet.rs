...
impl SelectionBackend for SelectorFleet {
    ...
    async fn select_and_reserve(&self, req: &SelectRequest) -> Result<SelectResponse> {
        ...
        // Record the booking's replica BEFORE the POST so that if the response is
        // lost after the replica booked, `free_reservation` still knows where to
        // send the release. Requires the caller to supply the reservation id (the
        // EPP always does); a selector-minted id is recorded post-response below.
        if let Some(rid) = req.reservation_id.clone() {
            self.reservations.lock().await.insert(rid, base.clone());
        }
        ...
        // Cover a selector-minted id that differs from (or was absent in) the
        // request; the booking lives on this replica (replica-sync only
        // propagates the load effect to peers).
        if let Some(rid) = selected.reservation_id.clone()
            && req.reservation_id.as_deref() != Some(rid.as_str())
        {
