...
fn build_embedded_backend(cfg: &EppConfig) -> Result<Arc<dyn SelectionBackend>> {
    tracing::info!(
...
fn build_embedded_backend(_cfg: &EppConfig) -> Result<Arc<dyn SelectionBackend>> {
    anyhow::bail!(
...
impl EndpointPicker for EppRouter {
    async fn pick(
        ...
        let resp = match self.backend.select_and_reserve(&select_req).await {
            Ok(resp) => resp,
            Err(e) => {
                ...
                if let Err(cleanup) = self.backend.free_reservation(&reservation_id).await {
