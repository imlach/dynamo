...
func SetupAll(mgr ctrl.Manager, opts Options) error {
	...
	isClusterWide := cfg == nil || cfg.Namespace.Restricted == ""
	if isClusterWide {
	...
	dgdHandler := webhookvalidation.NewDynamoGraphDeploymentHandler(mgr, opts.OperatorPrincipal, runtimeConfig.GroveEnabled)
	...
	if cfg != nil {
		gpuDiscoveryEnabled = ptr.Deref(cfg.GPU.DiscoveryEnabled, true)
	}
	...
	dgdDefaulter := webhookdefaulting.NewDGDDefaulter(opts.OperatorVersion, runtimeConfig.GroveEnabled)
